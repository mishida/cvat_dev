TODO：以下のメモは後日改めて書き換える

# チェックツールの使用方法

## shellcheckのインストール

- shellcheckはbashスクリプトのvalidationを行うツールです。

```
# インストール
brew install shellcheck

# shellcheckを使用してbinディレクトリ内のbashファイルをvalidate
bin/shellcheck
```

## hadolintのインストール

- hadolintはDockefile用のlinterです
...
- ベーシック認証が設定されています。ローカル環境ではユーザー名が「abc」、パスワードが「123」になります。

```
docker-compose build cvat && docker-compose up
```
